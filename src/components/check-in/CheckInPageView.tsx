...
import { CheckInPageSkeleton } from "@/components/check-in/CheckInPageSkeleton";
...
import {
  buildCheckInLinkHelpWhatsAppUrl,
...
function CheckInPageShell({ children }: { children: React.ReactNode }) {
  return (
    <div className="bg-gradient-premium flex min-h-screen flex-col items-center justify-center px-4 py-10">
      <div className="flex w-full max-w-md flex-col gap-4 rounded-xl border border-brand-periwinkle/70 bg-card p-6 shadow-sm">
        <div className="flex justify-center">
          <DigicaLogo href={null} showEst priority />
        </div>
        {children}
      </div>
    </div>
...
type CheckInPageViewProps = {
  identifier: string;
};

export function CheckInPageView({ identifier }: CheckInPageViewProps) {
  const [data, setData] = useState<CheckInPublicData | null>(null);
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      try {
        ...
        const result = (await response.json()) as CheckInPublicData & {
          error?: string;
        };
  ...
  const fabWhatsAppUrl = data?.program.name
    ...
    <>
      ...
      {isLoading ? (
        ...
        <div className="flex flex-col gap-4">
          ...
          <CheckInForm identifier={identifier} data={data} />
      ...
      <RegistrationWhatsAppFab href={fabWhatsAppUrl} />
    </>
  );
}
